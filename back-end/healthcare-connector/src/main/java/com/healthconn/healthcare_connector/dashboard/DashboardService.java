...
public class DashboardService {
    ...
    public DashboardStatsDto getAdminStats() {
        DashboardStatsDto baseStats = getStats();
        ...
        List<DashboardStatsDto.ProviderSummary> providerSummary = providers.stream()
                .map(p -> {
                    List<AuthorizationRequest> reqs = requestRepository.findByProviderIdOrderByCreatedAtDesc(p.getId());
                    long pApproved = reqs.stream().filter(r -> r.getStatus() == RequestStatus.APPROVED).count();
                    long pRejected = reqs.stream().filter(r -> r.getStatus() == RequestStatus.REJECTED).count();
                    long pPending  = reqs.stream().filter(r -> r.getStatus() == RequestStatus.SUBMITTED
        ...
        List<DashboardStatsDto.PayerSummary> payerSummary = payers.stream()
                .map(p -> {
                    List<AuthorizationRequest> reviewed = requestRepository.findByReviewedByIdOrderByReviewedAtDesc(p.getId());
                    long pApproved = reviewed.stream().filter(r -> r.getStatus() == RequestStatus.APPROVED).count();
                    long pRejected = reviewed.stream().filter(r -> r.getStatus() == RequestStatus.REJECTED).count();
        ...
        return new DashboardStatsDto(
                ...
                baseStats.submitted(),
                baseStats.underReview(),
                baseStats.approved(),
                baseStats.rejected(),
                baseStats.statusDistribution(),
                baseStats.monthlyByPriority(),
                providerSummary,
                payerSummary
        );
    }
