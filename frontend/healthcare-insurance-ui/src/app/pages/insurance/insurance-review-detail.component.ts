...
@Component({
  selector: 'app-insurance-review-detail',
  standalone: true,
  imports: [
  ...
  template: `
    <section class="page">
      <div class="header">
        <h2>Review Claim</h2>
        <a mat-button routerLink="/insurance/review">Back to Incoming Claims</a>
      </div>
      ...
      <div *ngIf="!loading && claim" class="detail-layout">
        ...
        <div class="info-grid">
          ...
          <mat-card class="info-card">
            ...
            <mat-card-content>
              <p><strong>Claim Amount:</strong> {{ claim.amount | currency }}</p>
          ...
          <mat-card class="info-card full-width">
            ...
            <mat-card-content>
              <p><strong>Healthcare Remarks:</strong> {{ claim.healthcareRemarks || 'None' }}</p>
              <p><strong>Insurance Remarks:</strong> {{ claim.insuranceRemarks || 'None' }}</p>
            </mat-card-content>
          </mat-card>
        ...
        <div class="decision-actions">
          <button mat-raised-button color="warn" (click)="reject()" [disabled]="!canAct">
            <mat-icon>close</mat-icon> Reject
          </button>
          <button mat-raised-button color="primary" (click)="openApproveDialog()" [disabled]="!canAct">
  ...
  styles: [`
    ...
    .decision-actions { display: flex; gap: 12px; justify-content: flex-end; margin-top: 10px; }
  `]
})
export class InsuranceReviewDetailComponent implements OnInit {
  ...
  private loadClaim(id: string): void {
    ...
    this.claimService.getById(id).subscribe({
      next: (c) => {
        this.claim = c;
        this.loadRelatedData(c);
        this.loading = false;
      },
      error: (err) => {
        console.error('Failed to load claim', err);
        this.notification.error('Failed to load claim.');
        this.loading = false;
      }
    });
  }
  ...
  private loadRelatedData(claim: ClaimDTO): void {
    this.patientService.get(claim.patientId).subscribe({
      next: (p) => {
        this.patient = p;
    ...
    this.policyService.getById(claim.policyId).subscribe({
      next: (pol) => {
        this.policy = pol;
      },
      error: () => {}
    ...
    this.diseaseService.get(claim.diseaseId).subscribe({
      next: (d) => (this.disease = d),
      error: () => {}
    });

    this.docService.listByClaimId(claim.id).subscribe({
      next: (docs) => (this.documents = docs),
      error: () => {}
    });

    this.canAct = ['UNDER_REVIEW', 'PENDING', 'SUBMITTED'].includes((claim.status || '').toUpperCase());
  }
  ...
  openApproveDialog(): void {
    const dialogRef = this.dialog.open(ApproveDialogComponent, {
      width: '400px',
      data: { amount: this.claim?.amount || 0, remarks: '', disabled: false }
    });

    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.reviewService.approve(this.claim!.id, {
          approvedAmount: result.amount,
          insuranceRemarks: result.remarks
        }).subscribe({
          next: () => {
          ...
          error: (err) => {
            ...
            this.notification.error('Failed to approve claim.');
          }
        });
      }
    });
  ...
  reject(): void {
    const dialogRef = this.dialog.open(RejectDialogComponent, {
      width: '400px',
      data: { remarks: '', disabled: false }
    });

    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.reviewService.reject(this.claim!.id, {
          insuranceRemarks: result.remarks
        }).subscribe({
          next: () => {
            ...
            if (this.claim) {
              ...
              this.claim.insuranceRemarks = result.remarks;
            }
          },
          error: (err) => {
            console.error('Failed to reject claim', err);
            this.notification.error('Failed to reject claim.');
          }
        });
      }
    });
...
@Component({
  ...
  template: `
    ...
    <mat-dialog-actions align="end">
      ...
      <button mat-raised-button color="primary" (click)="onConfirm()" [disabled]="form.invalid">Approve</button>
    </mat-dialog-actions>
  `,
  styles: [`
    .full-width { width: 100%; }
    mat-dialog-content { display: flex; flex-direction: column; gap: 16px; min-width: 350px; }
  `]
})
export class ApproveDialogComponent {
  private fb = inject(FormBuilder);
  private dialogRef = inject(MatDialogRef<ApproveDialogComponent>);
  data = inject(MAT_DIALOG_DATA) as { amount: number; remarks: string; disabled: boolean };
...
@Component({
  ...
  template: `
    ...
    <mat-dialog-actions align="end">
      <button mat-button (click)="onCancel()">Cancel</button>
      <button mat-raised-button color="warn" (click)="onConfirm()" [disabled]="form.invalid">Reject</button>
    </mat-dialog-actions>
  `,
  styles: [`
    .full-width { width: 100%; }
...
export class RejectDialogComponent {
  ...
  data = inject(MAT_DIALOG_DATA) as { remarks: string; disabled: boolean };
