...
@Component({
  ...
  template: `
    <section class="page">
      ...
      <ng-template #noData>
        <p class="muted">No rejected claims found.</p>
      </ng-template>
    </section>
  `,
  styles: [`
...
export class RejectedClaimsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loadRejectedClaims();
  }

  private loadRejectedClaims(): void {
    this.loading = true;
    this.claimService.listAll().subscribe({
      next: (data) => {
        const filtered = data.filter(c => (c.status || '').toUpperCase() === 'REJECTED');
      ...
      error: (err) => {
        ...
        this.notification.error('Failed to load rejected claims.');
        this.loading = false;
      }
    });
  }
