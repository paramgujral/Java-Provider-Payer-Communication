...
import {
  ...
  signal, OnInit, PLATFORM_ID
} from '@angular/core';
import * as Highcharts from 'highcharts';
import { HighchartsChartComponent } from 'highcharts-angular';
import { DashboardService, DashboardStats, ProviderSummary, PayerSummary } from './dashboard.service';
...
export class Dashboard implements OnInit {
  ...
  isPieEmpty = signal<boolean>(false);
  isBarEmpty = signal<boolean>(false);
  ...
  loadStats(): void {
    ...
    this.dashboardService.getStats().subscribe({
      next: (data) => {
        ...
        this.isPieEmpty.set(data.statusDistribution.every(s => s.count === 0));
